...
const viewAllEmployees = function () {
    pool.query("SELECT e.id, e.first_name, e.last_name, r.title, r.department, r.salary, e.manager_id AS manager from employees e JOIN ( SELECT r.id, r.title, d.name AS department, r.salary from roles r INNER JOIN departments d ON r.department = d.id) r ON e.role_id = r.id", (err, result) => {
        if (err) {
            console.log(err);
        }
        else if (result) {
...
const employeeChoices = async () => {
    let employees;
    pool.query("SELECT first_name, last_name FROM employees", (err, result) => {
        if (err) {
            console.log(err);
        }
        else if (result) {
...
const updateEmpRole = async function () {
    inquirer
        ...
        .then((answers) => {
        // Adding a new Employee to the database using an insert statement
        const role = pool.query(`SELECT id FROM roles where title = ${answers.role}`, (err, result) => {
            if (err) {
                console.log(err);
            }
            else if (result) {
        ...
        pool.query(`INSERT INTO employees (first_name, last_name, role_id) 
            VALUES (${answers.employee.firstName}, ${answers.employee.lastName}, ${role})`, (err, result) => {
            if (err) {
                console.log(err);
            }
            else if (result) {
...
const viewRoles = function () {
    ...
    pool.query(`SELECT r.id, r.title, d.name AS department, r.salary from roles r INNER JOIN departments d ON r.department = d.id`, (err, result) => {
        if (err) {
            console.log(err);
        }
        else if (result) {
...
const departmentChoices = async function () {
    let departments;
    pool.query("SELECT name FROM departments", (err, result) => {
        if (err) {
            console.log(err);
        }
        else if (result) {
...
const addRole = async function () {
    inquirer
        ...
        const department = pool.query(`SELECT id FROM departments where name = '${answers.department}'`, (err, result) => {
            if (err) {
                console.log(err);
            }
            else if (result) {
        ...
        pool.query(`INSERT INTO roles (title, salary, department) 
            VALUES (${answers.role}, ${answers.salary}, ${department})`, (err, result) => {
            if (err) {
                console.log(err);
            }
            else if (result) {
...
const addDepartment = function () {
    inquirer
        ...
        pool.query(`INSERT INTO departments (name) 
